...
import com.example.managebackend.config.DevConfig;
import com.example.managebackend.pojo.TokenData;
    ...
    public static void setDefaultHttpSecurity(HttpSecurity http, String... permitUrls) throws Exception {
        ...
        if (DevConfig.ENABLE_SECURITY) {
            http.authorizeRequests()
                    ...
                    .authenticationEntryPoint((request, response, authenticationException) -> response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "未认证"))
                    .accessDeniedHandler((request, response, accessDeniedException) -> response.sendError(HttpServletResponse.SC_FORBIDDEN, "拒绝"));
        } else {
