...
public class RoleController {
    ...
    public ResponseVO<String> addRole(@RequestBody Role role) {
        ...
        roleService.save(role.setStatus(1)
    ...
    public ResponseVO<List<Role>> getRoleList(@RequestParam(defaultValue = "") @ApiParam("搜索内容") String searchContent) {
        List<Role> roleList = roleService.list(
                new QueryWrapper<Role>().eq("status",1).and(
    ...
    public ResponseVO<String> updateRole(@RequestBody Role role) {
        ...
        if (role == null || !role.getStatus().equals(1)) {
            return ResponseVO.error();
        }
        try {
            //这部分代码没有看懂？？？为什么要见擦汗是否重名？
            roleService.existsByName(role.getName(), true);
        } catch (RuntimeException e) {
            return ResponseVO.error();
        }
    ...
    public ResponseVO<String> deleteRoles(@RequestParam List<Integer> ids) {
        ...
        roleList.forEach(role -> role.setStatus(0));
